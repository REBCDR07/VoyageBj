  ...
  return (
    <div className="flex flex-col items-center w-full px-2 sm:px-4">
      ...
      <div className="w-full overflow-x-auto pb-4">
        ...
        >
          {data.status === 'COMPLETED' ? (
            // --- ARRIVAL TICKET LAYOUT ---
            <div className="w-full h-full flex flex-col items-center justify-center p-8 bg-green-50 text-center relative overflow-hidden">
              ...
              <div className="z-10 bg-white p-6 rounded-3xl shadow-xl max-w-lg w-full border border-green-100">
                <div className="w-16 h-16 bg-[#008751] rounded-full flex items-center justify-center text-white mx-auto mb-4 shadow-lg shadow-green-200">
                  <Bus size={32} />
                </div>
                <h2 className="text-2xl font-black text-gray-900 mb-2">Vous êtes arrivé ! 🇧🇯</h2>
                <p className="text-gray-600 mb-4 leading-relaxed">
                  Merci d'avoir choisi <span className="font-bold text-[#008751]">VoyageBj</span> et voyagé à bord de <span className="font-bold text-gray-800">{data.companyName}</span>.
                </p>
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 mb-4">
                  <p className="font-bold text-gray-800 text-lg flex items-center justify-center gap-2">
                    {data.origin} <span className="text-gray-400">→</span> {data.destination}
                  </p>
                  <p className="text-xs text-gray-400 mt-1 uppercase font-bold tracking-widest">TRAJET EFFECTUÉ</p>
                </div>
                <p className="text-sm font-bold text-[#008751]">À très bientôt pour votre prochain voyage !</p>
              </div>

              <div className="absolute bottom-0 left-0 right-0 h-32 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10"></div>
            </div>
          ) : (
      ...
      <div className="mt-6 flex flex-col sm:flex-row gap-3 sm:gap-4 w-full max-w-md no-print" data-html2canvas-ignore="true">
        ...
        >
          {isDownloading === 'image' ? <Loader2 className="w-4 h-4 animate-spin" /> : <span>TÉLÉCHARGER IMAGE</span>}
        </button>
      </div>
    </div>
  );
